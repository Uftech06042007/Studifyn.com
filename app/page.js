    ...
    <main>
      ...
      <section className="stage">
        ...
        <h1 className="wip" aria-label="Work in progress">
          {'Work in progress'.split('').map((c, i) => (
            <span key={i} style={{ animationDelay: `${i * 0.07}s` }} aria-hidden="true">
              {c === ' ' ? ' ' : c}
            </span>
          ))}
        </h1>
...
function Kid() {
  return (
    <svg className="kid" viewBox="0 0 260 240" role="img" aria-label="A kid looking around and tapping the screen">
      {/* touch ripples on the "glass" */}
      <g className="ripples">
        <circle className="ripple" cx="196" cy="66" r="8" />
        <circle className="ripple r2" cx="196" cy="66" r="8" />
      </g>
      ...
      <g className="bob">
        ...
        {/* right arm tapping the screen */}
        <g className="arm">
          <path className="limb" d="M172 194 q26 -60 22 -126" />
          <circle className="skin hand" cx="194" cy="66" r="12" />
        </g>
